...
from selenium.common.exceptions import NoSuchElementException, ElementNotInteractableException, \
    ElementClickInterceptedException, ElementNotVisibleException, StaleElementReferenceException
...
def condense_reviews(reviews_directory_path):
    ...
    for file_name in os.listdir(reviews_directory_path):
        ...
        if file_name.endswith('.json') and not file_name.startswith(
                '.') and file_name != "all_reviews.json" and "_reviews" in file_name:
...
def main():
    ...
    for i, book_id in enumerate(books_to_scrape):
        try:
            ...
            if reviews:
                print(str(datetime.now()) + ' ' + script_name + ': Scraped ✨' + str(
                    len(reviews)) + '✨ reviews for ' + book_id)
